...
这是一个面向收费产品雏形的网站。页面围绕电商定制产品图生成，Express 后端提供 API、Prisma/PostgreSQL 持久化、上传图片落盘，并预留 ComfyUI 或其他生图服务接口。
...
- `server.js`: Express 后端，提供静态文件、API、账户/积分/任务持久化和上传存储
...
当前后端使用 Prisma 连接 PostgreSQL，账户、积分、支付状态、任务历史和邀请记录都会写入数据库。

上传图片会保存到 `uploads/`，任务记录会保存图片的公开 URL 和文件引用。
